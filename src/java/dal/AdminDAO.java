...
//        System.out.println(d.sendEmail((new User(0,"ds", "ds","ds", "ds", "ds", "20030201", true, "hophuongdongk15@gmail.com", "ds",new Role(2, ""), 0)),"123413"));
    ...
    public void update(User p) {
        String sql = "update [User] set fullName = ?, userEmail= ?, userAdress=?, userPhone=?, userBirth=?,userAvatar = ?,userGender = ? where userID= ?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
...
//    }
    ...
    public User getUser(String user, String pass) {
        AdminDAO d = new AdminDAO();
        String sql = "select * from [User] where userName=? and userPass=? and uStatusID = 1";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, user);
            st.setString(2, pass);
